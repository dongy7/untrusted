...
function startLevel(map) {
    ...
    // Create a barrier in the middle
    for (var x = 0; x < map.getWidth() / 2 - 2; x++) {
        map.placeObject(x, 10, 'barrier');
    }

    for (var x = map.getWidth() / 2 + 2; x < map.getWidth(); x++) {
        map.placeObject(x, 10, 'barrier');
    }

    var player = map.getPlayer();

    // set callback to launch arrow
    player.setPhoneCallback(function() {
        var x = player.getX();
        var y = player.getY();
        map.placeObject(x, y-1, 'arrow');
    });
